...
@Controller // 빈 등록, 컨트롤러 등록
@RequestMapping("/first/*") // fisrt로 무조건 시작
...
//    }
    ...
    * 이 때 맵의 키는 name 속성 값이 된다. */
    ...
    /* 3. @ModeAttribute를 이용하는 방법
    * DTO 같은 모델을 커맨드 객체로 전달 받는 어노테이션이다.
    * 또한 해당 인스턴스를 model에 담는 직업도 자동으로 일어난다.
    ...
    * Model 영역에 해당 key로 값이 추가 되는 경우 session에 자동으로 등록한다.*/
    @PostMapping("login2")
